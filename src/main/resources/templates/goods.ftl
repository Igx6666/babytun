...
<head>
    <meta charset="UTF-8">
    <title>测试jenkins-${goods.title}</title>
        ...
        <div class="layui-col-xs8" style="padding: 10px">
            <ul class="bb-info">
                ...
                <li style="color: darkgrey">
                    <span>宝宝团价：</span><span
                        style="font-size: 26px;color: orange;font-weight: bold">${goods.currentPrice?string('¥0.00')}</span>
                    <span style="text-decoration: line-through">${goods.originalCost?string('¥0.00')}</span>
                </li>

                <li>
                    <span>运费： 6.0元 <#if goods.isFreeDelivery==1> 满 58 元包邮</#if></span>
                </li>
                <li>
                    数量：
    ...
    <div class="layui-row">
        <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
            ...
            <div class="layui-tab-content">
                ...
                <div class="layui-tab-item layui-col-xs8 layui-col-xs-offset2" id="evaluates">
                </div>
            </div>
        </div>
    </div>
...
<script>
    //由于模块都一次性加载，因此不用执行 static.use() 来加载对应模块，直接使用即可：
    ;!function () {
        var layer = layui.layer
                , form = layui.form;
        var $ = layui.$;
        $.getJSON("/evaluates/${goods.goodsId?string('0')}", function (data) {
            for (var i = 0; i < data.length; i++){
                $("#evaluates").append("<h2>"+data[i].createTime+"  "+data[i].content)
            }
         });
    }();
</script>
